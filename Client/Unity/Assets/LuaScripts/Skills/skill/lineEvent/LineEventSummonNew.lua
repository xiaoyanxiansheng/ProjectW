...
---@class LineEventSummonNew : SkillLineEvent
LineEventSummonNew = Classnew("LineEventSummonNew" , SkillLineEvent)

function LineEventSummonNew:ctor(skill, eventType , config,timeLineIndex)
...
function LineEventSummonNew:OnReset()

end

-- 子类继承 LineEvent时间触发
function LineEventSummonNew:OnInvoke()
    ...
    DebugSKillLog(DebugSKillLogLayer.All,tostring(self),"LineEventSummonNew:OnInvoke " , self._unitid, self._count, skillId, self._life_time, self._maxlimit, self._attr_extend)
    if entity then
...
-- 子类继承 处理多段LineEvent
function LineEventSummonNew:OnUpdate()

end

function LineEventSummonNew:OnSkillFinish()
